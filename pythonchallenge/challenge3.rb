...
def find_the_seven(str)
  str.gsub!("\n", '')
  lowercase = ('a'..'z')
  uppercase = ('A'..'Z')
  str.each_char.each_cons(9) do |ary|
    outer = ary[1, 3] + ary[5, 3]
    inner = ary[4]
    outest = [ ary.first,  ary.last ]
    next if uppercase.include? inner
    next if outer.any?{|char| lowercase.include? char }
    next unless outest.all?{|char| lowercase.include? char }
    puts inner
  end
end
